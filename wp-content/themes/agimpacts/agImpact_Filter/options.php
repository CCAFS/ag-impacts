...
{
    $doi = $_REQUEST['doi'];
	$result = query("SELECT id,doi_article FROM wp_article WHERE doi_article LIKE '%$doi%' ORDER BY id ASC ");
...
function scaleQuery(){
	...
	$doi = $_REQUEST['doi'];
    $result = query("SELECT DISTINCT(e.spatial_scale) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . "e.spatial_scale LIKE '%%' "
                . "ORDER BY spatial_scale ASC ");
    if(count($result) != 0) {
	for ($i=0;$i<count($result);$i++){
...
function cropQuery(){
    ...
    $result = query("SELECT DISTINCT(e.crop) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . "e.crop LIKE '%%' "
...
function modelQuery(){ //impact model or multimodel
    ...
    $result = query("SELECT DISTINCT(e.impact_models) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . " e.impact_models LIKE '%%' "
                . " ORDER BY e.impact_models ASC");
    if(count($result) != 0) {
	for ($i=0;$i<count($result);$i++){
...
function baselineQuery(){
	...
	if($suboption == 1){$baseline =" e.base_line_start "; $baselineforresult="base_line_start";}
	else if($suboption == 2){$baseline =" e.base_line_end ";$baselineforresult="base_line_end";}
    $result = query("SELECT DISTINCT(".$baseline.") "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . $baseline ." LIKE '%%' "
                . "ORDER BY ".$baseline." ASC ");
    if(count($result) != 0) {
	for ($i=0;$i<count($result);$i++){
...
function periodQuery(){ 
	...
	$suboption=$_REQUEST['suboption'];
	if($suboption == 1){$period =" e.projection_start "; $periodforresult="projection_start";}
	else if($suboption == 2){$period =" e.projection_end ";$periodforresult="projection_end";}
    $result = query("SELECT DISTINCT(".$period.") "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . $period." LIKE '%%' "
...
function countryQuery(){
    ...
    $result = query("SELECT DISTINCT(e.country) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . "e.country LIKE '%%' "
...
function subcontinentsQuery(){
	...
	$where = " ";
	if($country != ""){$where = $where." e.country ='".$country."' AND ";}

    $result = query("SELECT DISTINCT(e.region) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . $where
...
function climateQuery(){
    ...
    $result = query("SELECT DISTINCT(e.climate_scenario) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . "e.climate_scenario LIKE '%%' "
                . "ORDER BY e.climate_scenario ASC ");
    if(count($result) != 0) {
	for ($i=0;$i<count($result);$i++){
...
function adaptationQuery(){
    ...
    $result = query("SELECT DISTINCT(e.adaptation) "
                . "FROM wp_estimate e "
                . "INNER JOIN wp_article a ON e.article_id=a.id "
                . "WHERE "
                . "e.adaptation LIKE '%%' "
                . "ORDER BY e.adaptation ASC ");
    if(count($result) != 0) {
	for ($i=0;$i<count($result);$i++){
...
function dataTable(){
	...
	$tr = "";
	$result= query("SELECT a.id,e.idEstimate,a.doi_article,e.spatial_scale,e.crop,e.impact_models,"
